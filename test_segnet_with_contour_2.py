...
for o_i in range(len(output_)):
    ...
    for art_x in [2, 4, 5, 6, 7, 8, 9, 10, 11, 12]:
        # print art_x,
        o_x = output[:,:,art_x]
        p_thr = mean(o_x[o_x>=0.01])
        paths, path_values = get_greedy_contours(o_x, p_thr)
        path_idx = np.argmax([sum(i) for i in path_values])
        path = paths[path_idx]
        # this gives the longest path - next find the biggest path which doesnt overlap with this one
        overlap_arr = np.array([len(set(paths[path_idx]).intersection(set(paths[i]))) for i in range(len(paths))])
        non_overlap_idxs = np.where(overlap_arr==0)[0]


        # if art_x==2: paths, path_values = get_greedy_contours(output[:,:,art_x], 0.2, greedier=False)
...
O = np.zeros((w,h))
for i in range(1,13):
    out_i = output[:,:,i].reshape((w,h)).copy()
    out_i+=i
    out_i[out_i<=i+0.1]=0
    O+=out_i